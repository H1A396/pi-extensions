...
- **展示**：免费总额列 = 最近一次校准余额（用户无法准确计算免费总额模型，以 dashboard 读数为权威总额）；
  已用列 = 校准时刻起的消耗；剩余列 = 校准余额 − 已用。未校准过则回退为「$20 + $10×月数」模型估算
- **校准提醒**：/myqy-web-tools-quota 与 /myqy-web-tools-usage 报表末尾均附带校准提示（Exa 无官方接口，建议定期对照 dashboard 校准）
